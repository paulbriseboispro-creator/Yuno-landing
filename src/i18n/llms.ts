...
export function landingMarkdown(lang: LandingLang): string {
  ...
  out.push(`## ${h.compare}`, "");
  // Feature matrix: one row per feature, one column per platform.
  const cols = [t.compare.yuno, ...t.compare.competitors];
  const cell = (c: { mark: string; note: string }) =>
    `${t.compare.legend[c.mark as keyof typeof t.compare.legend]}${c.note ? ` (${c.note})` : ""}`;
  out.push(`${t.compare.title} ${t.compare.sub}`, "");
  out.push(
    `| | ${cols.map((c) => c.name).join(" | ")} |`,
    `|---|${cols.map(() => "---").join("|")}|`,
  );
  t.compare.features.forEach((f, i) =>
    out.push(`| ${f} | ${cols.map((c) => cell(c.cells[i])).join(" | ")} |`),
  );
  out.push(`| ${t.compare.costLabel} | ${cols.map((c) => c.cost).join(" | ")} |`);
  out.push("", `**${t.compare.allInOne.title}** ${t.compare.allInOne.sub}`, "");
  for (const it of t.compare.allInOne.items) out.push(`- **${it.title}:** ${it.body}`);
  out.push("", `_${t.compare.footnote}_`, "");
