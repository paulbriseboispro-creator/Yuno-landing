...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn("relative w-full max-w-xs rounded-xl border bg-background p-1", className)}
      {...props}
    />
  );
}
...
function Badge({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      className={cn("rounded-full border bg-background px-3 py-1 text-xs shadow-xs", className)}
      {...props}
    />
  );
}
...
function MainPrice({ className, ...props }: React.ComponentProps<"span">) {
  return <span className={cn("font-extrabold text-3xl tracking-tight", className)} {...props} />;
}

function Period({ className, ...props }: React.ComponentProps<"span">) {
  return <span className={cn("pb-1 text-muted-foreground text-sm", className)} {...props} />;
}
