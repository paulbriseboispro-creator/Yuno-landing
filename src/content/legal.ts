// Legal page copy (Privacy + Terms), available in every locale. Routes read the
// raw `legalContent[locale]` map inside head() for meta/SEO; components read it
// via useLegal(). French is checked against the English shape at compile time
// (LegalContent = typeof en), so keys and section array lengths must match.
import { useLocale, type Locale } from "@/i18n/locale";

const en = {
  privacy: {
    meta: {
      title: "Privacy Policy — Yuno",
      description:
        "How Yuno collects, uses and protects personal data for clubs, organizers, promoters and their guests.",
    },
    heading: "Privacy Policy",
    lastUpdated: "Last updated: June 9, 2026",
    badge: "Legal",
    placeholder: "This is a placeholder document.",
    notice:
      "The final, lawyer-reviewed Privacy Policy is on the way. In the meantime, for any question about how Yuno handles your data, write to",
    sections: [
      {
        title: "What we collect",
        body: "Account data (name, email, role), operational data generated by your use of Yuno (events, orders, table reservations), and the minimum guest data required to deliver tickets and Click & Collect orders.",
      },
      {
        title: "How we use it",
        body: "To run the platform, process payments through Stripe Connect, send transactional notifications, and provide analytics dashboards to venue operators, organizers and promoters.",
      },
      {
        title: "Audience measurement",
        body: "This site uses PostHog (EU-hosted) to understand how it is used: pages viewed, clicks and the steps of the sign-up form. Nothing is stored on your device — no cookie, no local storage — and no email, phone number or name is sent. If you create a Yuno account here, this visit is linked to your account's internal identifier.",
      },
      {
        title: "Your rights",
        body: "Access, rectification, deletion, portability and objection rights under the GDPR. Email us to exercise any of them.",
      },
    ],
  },
  terms: {
    meta: {
      title: "Terms of Service — Yuno",
      description:
        "The terms governing use of the Yuno platform by clubs, organizers, promoters and their guests.",
    },
    heading: "Terms of Service",
    lastUpdated: "Last updated: June 9, 2026",
    badge: "Legal",
    placeholder: "This is a placeholder document.",
    notice:
      "The final, lawyer-reviewed Terms of Service are on the way. For any question in the meantime, contact",
    sections: [
      {
        title: "Using Yuno",
        body: "By creating an account you agree to operate within the platform's intended use: hosting and operating nightlife events, processing ticket sales and bar / VIP transactions in compliance with local law.",
      },
      {
        title: "Fees & payments",
        body: "Plan and transactional fees are described on the Pricing page. Payments are processed through Stripe Connect; each connected account is bound by Stripe's own terms.",
      },
      {
        title: "Liability",
        body: 'Yuno provides the platform "as is" while we finalize service-level commitments. Operators remain responsible for ticket validity, capacity enforcement and on-site decisions.',
      },
    ],
    pricingLink: "Pricing page",
  },
};

export type LegalContent = typeof en;

const fr: LegalContent = {
  privacy: {
    meta: {
      title: "Politique de confidentialité — Yuno",
      description:
        "Comment Yuno collecte, utilise et protège les données personnelles des clubs, organisateurs, promoteurs et de leurs invités.",
    },
    heading: "Politique de confidentialité",
    lastUpdated: "Dernière mise à jour : 9 juin 2026",
    badge: "Mentions légales",
    placeholder: "Ceci est un document provisoire.",
    notice:
      "La version finale de la Politique de confidentialité, validée par nos avocats, arrive bientôt. En attendant, pour toute question sur la façon dont Yuno traite vos données, écrivez à",
    sections: [
      {
        title: "Ce que nous collectons",
        body: "Les données de compte (nom, e-mail, rôle), les données opérationnelles générées par votre utilisation de Yuno (événements, commandes, réservations de tables) et le minimum de données invités nécessaires à la délivrance des billets et des commandes Click & Collect.",
      },
      {
        title: "Comment nous l'utilisons",
        body: "Pour faire fonctionner la plateforme, traiter les paiements via Stripe Connect, envoyer les notifications transactionnelles et fournir des tableaux de bord d'analyse aux exploitants d'établissements, organisateurs et promoteurs.",
      },
      {
        title: "Mesure d'audience",
        body: "Ce site utilise PostHog (hébergé dans l'UE) pour comprendre comment il est utilisé : pages vues, clics et étapes du formulaire d'inscription. Rien n'est stocké sur votre appareil — ni cookie, ni stockage local — et aucun email, téléphone ni nom n'est transmis. Si vous créez un compte Yuno ici, cette visite est reliée à l'identifiant interne de votre compte.",
      },
      {
        title: "Vos droits",
        body: "Droits d'accès, de rectification, de suppression, de portabilité et d'opposition au titre du RGPD. Écrivez-nous pour exercer l'un de ces droits.",
      },
    ],
  },
  terms: {
    meta: {
      title: "Conditions d'utilisation — Yuno",
      description:
        "Les conditions régissant l'utilisation de la plateforme Yuno par les clubs, organisateurs, promoteurs et leurs invités.",
    },
    heading: "Conditions d'utilisation",
    lastUpdated: "Dernière mise à jour : 9 juin 2026",
    badge: "Mentions légales",
    placeholder: "Ceci est un document provisoire.",
    notice:
      "La version finale des Conditions d'utilisation, validée par nos avocats, arrive bientôt. Pour toute question en attendant, contactez",
    sections: [
      {
        title: "Utilisation de Yuno",
        body: "En créant un compte, vous acceptez d'utiliser la plateforme conformément à sa finalité : organiser et exploiter des événements de vie nocturne, traiter les ventes de billets et les transactions bar / VIP dans le respect de la législation locale.",
      },
      {
        title: "Frais & paiements",
        body: "Les frais d'abonnement et de transaction sont décrits sur la page Tarifs. Les paiements sont traités via Stripe Connect ; chaque compte connecté est soumis aux propres conditions de Stripe.",
      },
      {
        title: "Responsabilité",
        body: "Yuno fournit la plateforme « en l'état » pendant que nous finalisons nos engagements de niveau de service. Les exploitants restent responsables de la validité des billets, du respect de la capacité et des décisions prises sur place.",
      },
    ],
    pricingLink: "page Tarifs",
  },
};

export const legalContent: Record<Locale, LegalContent> = { en, fr };

export function useLegal(): LegalContent {
  return legalContent[useLocale()];
}
